<script>
    ...
    function removeClass(arr, isLeft) {
      for (let n of arr) {
        ...
        const el = document.querySelector(
          `[data-boxid="${platePos}-${currentIndex}"]`
        );
      ...
      if (ballPrevRowPosition > row) {
        let ballBounceInterval = setInterval(() => {
          if (ballPrevColPosition > ballLastColPosition) {
            nextBall = document.querySelector(
              `[data-boxid="${row - 1}-${col - 1}"]`
            );
            // nextBall.classList.add("ball")
            document
              .querySelector(`[data-boxid="${row}-${col}"]`)
    ...
    function bounceDirection(row, col, direction) {
      let balldropInterval = setInterval(() => {
        ...
        nextBall = document.querySelector(
          `[data-boxid="${row - 1}-${currentCol}"]`
        );
  
        if (direction === directions.middle)
          document
            .querySelector(`[data-boxid="${row}-${currentCol}"]`)
            .classList.remove("ball");
        else if (direction === directions.left) {
          document
            .querySelector(`[data-boxid="${row}-${currentCol + 1}"]`)
            .classList.remove("ball");
        ...
        if (nextBall.classList.contains("prey-box")) {
      return;
        }
    ...
    function ballEatsPrey(){
  
    }
  
    const specialItems = [];
  
    function createPrey(arr) {
      // preys that disappers after ball touches them and bounce back the ball downwards
      for (let i of arr) {
        for (let j = 2; j < 30; j++) {
          const el = document.querySelector(`[data-boxid="${i}-${j}"]`);
          el.classList.add("prey-box");
  
          if (specialItems.length < 6) {
            let specialNumCol = Math.floor(Math.random() * (29 - 2 + 1) + 2);
            let specialNumRow = Math.floor(Math.random() * (5 - 2 + 2) + 2);
            specialItems.push([specialNumRow,specialNumCol]);
            let specialitem = document.querySelector(
              `[data-boxid="${specialNumRow}-${specialNumCol}"]`
            );
            specialitem.classList.add("special-box");
          }
        }
      }
    }
        ...
        {#each arr as j}
          ...
          {:else if j === 1}
            <div
              ...
              data-column={i}
              data-boxid="{j}-{i}"
              class="box wall wall-horizontal"
            />
          {:else}
